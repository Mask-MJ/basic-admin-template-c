...
import type { SaasButtonGroupProps, ValueType } from './button';

import { computed, ref, watch } from 'vue';

import { Circle, CircleCheckBig, LoaderCircle } from '@saas-core/icons';
import { cn, isFunction } from '@saas-core/shared/utils';

import { objectOmit } from '@vueuse/core';

import { SaasRenderContent } from '../render-content';
import SaasButtonGroup from './button-group.vue';
import Button from './button.vue';

const props = withDefaults(defineProps<SaasButtonGroupProps>(), {
  gap: 0,
  multiple: false,
  showIcon: true,
  size: 'middle',
  ...
  >
    ...
    >
      ...
      <slot name="option" :label="btn.label" :value="btn.value" :data="btn">
        <SaasRenderContent :content="btn.label" />
      </slot>
    </Button>
  </SaasButtonGroup>
</template>
<style lang="scss" scoped>
.saas-check-button-group {
  display: flex;
  flex-wrap: wrap;
