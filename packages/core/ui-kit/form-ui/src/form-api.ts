...
import type { Recordable } from '@saas-core/typings';

import type { FormActions, FormSchema, SaasFormProps } from './types';

import { isRef, toRaw } from 'vue';

import { Store } from '@saas-core/shared/store';
import {
  bindMethods,
  createMerge,
  formatDate,
  ...
  StateHandler,
} from '@saas-core/shared/utils';

function getDefaultState(): SaasFormProps {
  return {
    actionWrapperClass: '',
    collapsed: false,
    collapsedRows: 1,
...
export class FormApi {
  // private api: Pick<SaasFormProps, 'handleReset' | 'handleSubmit'>;
  public form = {} as FormActions;
  isMounted = false;

  public state: null | SaasFormProps = null;
  stateHandler: StateHandler;

  public store: Store<SaasFormProps>;
  ...
  private prevState: null | SaasFormProps = null;

  constructor(options: SaasFormProps = {}) {
    const { ...storeState } = options;

    const defaultState = getDefaultState();

    this.store = new Store<SaasFormProps>(
      {
        ...defaultState,
        ...storeState,
      },
  ...
  setState(
    stateOrFn:
      | ((prev: SaasFormProps) => Partial<SaasFormProps>)
      | Partial<SaasFormProps>,
  ) {
  ...
  private async getForm() {
    ...
    if (!this.form?.meta) {
      throw new Error('<SaasForm /> is not mounted');
    }
    return this.form;
  }
