...
import { isMacOs, merge } from '@saas-core/shared/utils';
...
class PreferenceManager {
   ...
   * overrides  要覆盖的偏好设置
   */
  public async initPreferences({ overrides }: InitialOptions) {
    // 是否初始化过
    ...
    const mergedPreference = merge(
      {},
      // overrides,
      this.loadCachedPreferences().value,
  ...
  private _savePreferences(preference: Preferences) {
    useStorage(STORAGE_KEY, preference);
    useStorage(STORAGE_KEY_LOCALE, preference.app.locale);
    useStorage(STORAGE_KEY_THEME, preference.theme.mode);
  }
  ...
  private loadCachedPreferences() {
    return useStorage<Preferences>(STORAGE_KEY, defaultPreferences);
  }
  ...
  private loadPreferences(): Preferences {
    return this.loadCachedPreferences().value;
  }
